...
"turf_jungle","turf_swamp","turf_volcano","turf_tidalmarsh","turf_meadow",
 "hail_ice","sand",
 --"coconut",
 "coffe_beans_raw", "coffe_beans", "cutwheat", "tee", "tee_g", "tee_m", "tee_s", "tee_r", "tee_r2",
...
--return self.inst.components.health:GetPercent() < 0.15 end,
	...
	--WhileNode(function() return self.inst.components.health:GetPercent() < 0.2 end, "LowHealth", StandStill(self.inst, CanStandStill, Follow(self.inst, function() return self.inst.components.follower.leader end, 1, 2, 3 ))),
		WhileNode( function() return Peace_check(self.inst) and self.inst.components.health:GetPercent() < 0.3 and self.inst.components.combat.target == nil or not self.inst.components.combat:InCooldown() end, "AttackMomentarily", ChaseAndAttack(self.inst, 8,12)),
              WhileNode( function() return Peace_check(self.inst) and self.inst.components.health:GetPercent() < 0.3 and self.inst.components.combat.target and self.inst.components.combat:InCooldown() end, "Dodge",
                    RunAway(self.inst, function() return self.inst.components.combat.target end, 3, 10) ),
					WhileNode( function() return Peace_check(self.inst) and self.inst.components.health:GetPercent() >= 0.3 end, "AttackMomentarily", ChaseAndAttack(self.inst, 9,14)),
