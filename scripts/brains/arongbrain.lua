...
WhileNode( function() return Peace_check(self.inst) and self.inst.components.health:GetPercent() < 0.3 and self.inst.components.combat.target == nil or not self.inst.components.combat:InCooldown() end, "AttackMomentarily", ChaseAndAttack(self.inst, 8,12)),
              WhileNode( function() return Peace_check(self.inst) and self.inst.components.health:GetPercent() < 0.5 and self.inst.components.combat.target and self.inst.components.combat:InCooldown() end, "Dodge",
                    RunAway(self.inst, function() return self.inst.components.combat.target end, 3, 10) ),
					WhileNode( function() return Peace_check(self.inst) and self.inst.components.health:GetPercent() >= 0.5 end, "AttackMomentarily", ChaseAndAttack(self.inst, 9,14)),					
