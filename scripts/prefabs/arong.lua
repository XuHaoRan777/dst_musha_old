...
local function CanShareTarget(dude)
    return (dude:HasTag("beefalo") or dude:HasTag("companion") or dude:HasTag("musha"))
...
inst.components.domesticatable:DeltaObedience(1.0)
...
inst.components.domesticatable:DeltaObedience(0.3)
    if math.random() < poopchance then
...
if inst.crazyness then 
inst.peace = true
elseif not inst.crazyness then
...
if leader and inst.follow then
inst.components.combat:GiveUp()
inst.components.health.externalabsorbmodifiers:SetModifier(inst, .95)
inst.components.locomotor.walkspeed = 2.2
elseif leader and not inst.follow then
...
return Prefab("arong", arong, assets, prefabs),
       Prefab("arong_carry", arong_carry, assets, prefabs)