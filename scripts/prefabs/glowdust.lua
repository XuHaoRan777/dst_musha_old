...
eater.components.fatigue_sleep:DoDelta(-20)
eater.components.stamina_sleep:DoDelta(10)
end
if eater.components.spellpower then
eater.components.spellpower:DoDelta(20)
end
 
create_light(eater, "glow_light")
end
...
local function itemfn()
   ...
   local light = inst.entity:AddLight()
	...
	--local swap_data = {bank = "ashes", anim = "idle"}
    --MakeInventoryFloatable(inst, "small", 0.05, 0.80, nil, nil, swap_data)
	MakeInventoryFloatable(inst, "small", 0.05, {1, 1.5, 1})
    ...
    inst.components.edible.foodtype = FOODTYPE.SEEDS --FOODTYPE.VEGGIE
...
return Prefab( "glowdust", itemfn, assets),
       Prefab("glow_light", lightfn, nil, lightprefabs)
