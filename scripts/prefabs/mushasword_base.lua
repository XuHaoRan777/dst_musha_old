...
TakeItem_effect(inst)
	...
	-----
	if  math.random() < flamechance and target.components.burnable and not target:HasTag("structure") and not target:HasTag("stalkerminion") and not target:HasTag("smashable") and not target:HasTag("wall") and not target:HasTag("alignwall") and not target:HasTag("shadowminion") and target.components.health and not target.components.health:IsDead() and not inst.boost and inst.level >=210 then
...
local function fn()
  ...
  MakeInventoryPhysics(inst)  
   ...
   inst.entity:AddMiniMapEntity()
	...
	local swap_data = {bank = "nightmaresword", anim = "anim"}
