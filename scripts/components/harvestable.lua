...
function Harvestable:Harvest(picker)
    if self:CanBeHarvested() then
		...
		if self.product ~= nil then
			if picker ~= nil and not picker:HasTag("yamcheb") and not picker.critter_musha and picker.components.inventory ~= nil then
				picker:PushEvent("harvestsomething", { object = self.inst })
			end
			if picker ~= nil and (picker:HasTag("yamcheb") or picker.critter_musha) and picker.components.container ~= nil then
				picker:PushEvent("harvestsomething", { object = self.inst })
			end
			...
			for i = 1, produce, 1 do
				...
				if loot ~= nil then
					...
					elseif picker ~= nil and picker.components.container ~= nil and (picker:HasTag("yamcheb") or picker.critter_musha) then
							picker.components.container:GiveItem(loot, nil, pos)
					else
